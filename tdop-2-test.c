...
enum eqn_token {
	...
	EQN_SEQ_TOK, EQN_LPAR_TOK, EQN_RPAR_TOK, EQN_SET_TOK, EQN_PAIR_TOK,
	EQN_COND_TOK, EQN_COLON_TOK, EQN_PLUS_TOK, EQN_MINUS_TOK, EQN_STAR_TOK,
	EQN_CARET_TOK, EQN_FACT_TOK,
	EQN_TOKEN_COUNT
};
...
static int get_token (int c)
{
	switch (c) {
	case EOF:	return EOS;
	case '!':	return EQN_FACT_TOK;
	case '(':	return EQN_LPAR_TOK;
	case ')':	return EQN_RPAR_TOK;
	case '*':	return EQN_STAR_TOK;
	case '+':	return EQN_PLUS_TOK;
	case ',':	return EQN_PAIR_TOK;
	case '-':	return EQN_MINUS_TOK;
	case ':':	return EQN_COLON_TOK;
	case ';':	return EQN_SEQ_TOK;
	case '=':	return EQN_SET_TOK;
	case '?':	return EQN_COND_TOK;
	case '^':	return EQN_CARET_TOK;
...
#define LED_SEQ_BOUND	(10 + 0)	/* right assoc */

/*
 * exp -> exp a, ';', exp b : (seq a b)
 */
static struct se_node *led_seq (FILE *in, struct se_node *a)
{
	struct se_node *b;

	if ((b = eqn_parse_exp (in, LED_SEQ_BOUND)) == NULL)
		goto ep2;
...
#define LED_SET_BOUND	(20 + 0)	/* right assoc */

/*
 * exp -> exp a, '=', exp b : (set a b)
 */
static struct se_node *led_set (FILE *in, struct se_node *a)
{
	struct se_node *b;

	if ((b = eqn_parse_exp (in, LED_SET_BOUND)) == NULL)
		goto ep2;
...
#define LED_PAIR_BOUND	(30 + 0)	/* right assoc */

/*
 * exp -> exp a, ',', exp b : (pair a b)
 */
static struct se_node *led_pair (FILE *in, struct se_node *a)
{
	struct se_node *b;

	if ((b = eqn_parse_exp (in, LED_PAIR_BOUND)) == NULL)
		goto ep2;
...
#define LED_STAR_BOUND	(60 + 1)	/* left assoc */

/*
 * exp -> exp a, '*', exp b : (mul a b)
 */
static struct se_node *led_star (FILE *in, struct se_node *a)
{
	struct se_node *b;

	if ((b = eqn_parse_exp (in, LED_STAR_BOUND)) == NULL)
		goto ep2;
...
#define LED_CARET_BOUND	(80 + 0)	/* right assoc */

/*
 * exp -> exp a, '^', exp b : (pow a b)
 */
static struct se_node *led_caret (FILE *in, struct se_node *a)
{
	struct se_node *b;

	if ((b = eqn_parse_exp (in, LED_CARET_BOUND)) == NULL)
		goto ep2;
...
static const struct tdop_entry table[EQN_TOKEN_COUNT] = {
	[EQN_SEQ_TOK]	= { NULL,	led_seq,	10	},
	[EQN_LPAR_TOK]	= { nud_lpar,	NULL,			},
	[EQN_SET_TOK]	= { NULL,	led_set,	20	},
	[EQN_PAIR_TOK]	= { NULL,	led_pair,	30	},
	[EQN_COND_TOK]	= { NULL,	led_cond,	40	},
	[EQN_PLUS_TOK]	= { nud_plus,	led_plus,	50	},
	[EQN_MINUS_TOK]	= { nud_minus,	led_minus,	50	},
	[EQN_STAR_TOK]	= { NULL,	led_star,	60	},
	[EQN_CARET_TOK]	= { NULL,	led_caret,	80	},
